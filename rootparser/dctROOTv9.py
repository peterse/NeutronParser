#v9: Implemented ~fast~ range finding with histogram drawing methods
#v8 Reworking construct_all_ranges() etc
...
#get_ranges_local - add write protocol to totally circumvent brute force?
...
class dctTools():
	...
	def __init__(self, dct, debug=False):
		...
		self.debug = debug
		#get_best_ranges params
		self.fast_bad_tripped = False #Indicates whether the FT channel-counting methods messed up
		self.fast_bad_warning = False
		self.bad_br = []
	...
	# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	def printer(self, obj=None, started=False, trim=True, prec = 2):
	#Prints a dictionary's contents, with some formatting for readability
	#prec is the digits after the decimal to be printed IN PROGRESS
		if not started:
			self.spacing = 0
			obj = self.dct
			...
			for count, key in enumerate(obj):
				...
				#Print out lines for bottom layer of dct
					...
					if count > 20 and trim:
						...
						break
					global trim_what
					print "".join([" "]*self.spacing),"%s: %s" % (key, trim_what(obj[key], prec))
	...
	# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	def get_best_ranges(self, *args, **quietarg):
	...
	#Or creates a dictionary of most extreme ranges for trees for all instances in *args
		...
		quiet = quietarg.pop("quiet", True)
		temp_tree = " "
		#Constructing ranges at the "tree level"; individual DT instance
		if not any(args):
			#temp is a dictonary of leafs with lists of all min and max ranges found for each channel
			self.temp = {}

			for i, (tree_name, branches) in enumerate(self.dct.iteritems()):
				if tree_name != temp_tree:
					print tree_name, "XXXXXXXXXXXXXXXXX"
				temp_tree = tree_name
					...
					for br_name, channels in branches.iteritems():
						for leaf_num in channels:
							...
							try:
								...
								if not self.dct[tree_name][br_name][leaf_num] and self.debug:
									continue
								...
								try:
									...
									#Gather inconstistent branches from all
									self.fast_bad_tripped = True
									self.bad_br.append(br_name)
					if any(self.bad_br):
						print "Warning: Ranges have been constructed such that the number of subleaf dimensions vary. The following branches will have incomplete histogram ranges:"
						self.bad_br = list(set(self.bad_br))
						print "\n".join(self.bad_br)

								#print "Fatal error in fileTools().get_best_ranges; N_channels_dct returned inconsistent leaf dims for non fast_channel_count counting method. Please contact dctROOT admin he'll work it out"
					...
					for br_name, channels in branches.iteritems():
						...
						if br_name not in self.temp.keys():
							...
							for leaf_num in channels:
								self.temp[br_name][leaf_num] = [[], []]
						#Skip bad branches
						if br_name in self.bad_br:
							continue
						for leaf_num in channels:
							if not self.dct[tree_name][br_name].get(leaf_num):
								self.bad_br.append(br_name)
								continue
							#The bottom of temp is a running list of extrema
							try:
			...
			for br_name in self.temp:
				for leaf_num, minmax in self.temp[br_name].iteritems():
					#if self.debug and not minmax[0]:
						#continue
					#If fast_bad_tripped was set to True, there are inconsistent leaf dimension and the user knows it
					if (not minmax[0]) and self.fast_bad_tripped:
						continue
					try:
		...
		#Constructing ranges at the "file level": Master ranges from set of DT instances
		elif any(args):
			...
			self.get_best_ranges(quiet=False)
...
#llocal - if True, ignores some passive .db functions DOES NOT CURRENTLY AFFECT USE OF .DB METHODS
...
#get_ranges_local() - returns a minmax_dct of all trees in the file
...
class fileTools():
	...
	def __init__(self, file_name, rfile_path=None,  get_leafs=False, fast_channel_count=True, local=False, debug=False):
		self.tiers = 0
		...
		if rfile_path:
			...
			print "Setting rootfile path to current working directory: %s" % os.getcwd()
			self.rfile_path = os.getcwd()
		self.status_good = self.check_path(self.rfile_path)
		...
		if ".root" in file_name:
			#Public attributes
			self.file_handle = R.TFile.Open(self.file_name, 'read')
			self.mode = "ROOT"
			self.exp_dct = {}
			self.N_channels_dct = {}
			self.tree_dct = {}
			...
			#Reconstruct/fix db if databases are relevant
			if self.sister_db_exists and not self.local:
			...
			self.sister_rootfile = self.get_rootfile_name(file_name)
	...
	#For context management when accessing handles using this FT
	def __enter__(self):
		self.env = R.TFile.Open(self.file_name, 'r')
		return R.TFile.Open(self.file_name, 'r')
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	...
	#Construct a dictionary with parallel structure to the ROOT file (named at initialization)
		...
		temp_names = []
		#Account for non-unique entry names
		#if obj_name in dct:
			#obj_name += "x"
		...
		if obj_type in ["<class 'ROOT.TFile'>", "<class '__main__.TKey'>", "<class 'ROOT.TDirectoryFile'>"]:
			#print obj_name, obj_handle
			#print dct[obj_name]
			obj_iter = obj_handle.GetListOfKeys()
			for sub in obj_iter:
				#Deal with trees with the same name
				if sub.GetName() in temp_names:
					#print "Copy found!"
	...
	#Iterate over all trees in the file to produce a complete db
		...
		with cd("%s/%s" % (self.rfile_path, self.rfile_directory)):
			...
			for i_tree, tree in enumerate(self.trees):
				try:
					self.range_construct(dbname_out, tree, i_tree, auto=True, fast=self.fast_channel_count)
				except:
	...
	def get_rfile_name(rootfile_name):
		...
		return name
	@staticmethod
	def get_rootfile_name(rootfile_name):
		name = rootfile_name.replace("_ranges.db" ,".root")
		return name
	# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	def range_construct(self, dbname_out, tree, i_tree, auto=False, fast=False):
	#Find ranges for leafs and populate an SQLite3 db
	#Option append will add a new table to an existing database
	#fast (self.fast_channel_count) will use local ROOT methods for rangefinding
		...
		#Fast method
		if fast:
			self.get_ranges_local()
			minmax_dct = self.tree_dct[tree.GetName()]
		#Brute force method
		else:
	...
	# - - - - - - - - -  - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	def get_ranges_local(self, mode="max", Ndev=None):
	#Alternative to construct_all_ranges(): No database interxn, populates tree_dct
	#mode="max" gives histogram boundaries, mode="stats" gives ranges within Ndev std deviations of the mean value
		#Set the proper ranging function
		if mode == "max":
			bound_func = self.max_range
		elif mode == "stats":
			bound_func = self.stat_range
		#Use ROOT built-ins, DO NOT write to database (good for local stuff)
		if self.tree_dct_pop:
			return self.tree_dct
		print "fileTools(%s,debug=%s).get_ranges_local(%s) finding leaf ranges via ROOT methods..." % (self.file_name, self.DEBUG, mode)
		for tree_name in self.tree_dct:
			tree = self.tree_handle_dct[tree_name]
			for br_name in self.tree_dct[tree_name]:
				N_sl = self.N_channels_dct[tree_name][br_name]
				#print br_name, N_sl
				#use minima/maxima functions on subleaf histogram objects
				if N_sl < 100 or N_sl == 8256:
					for leafnum in xrange(N_sl):
						if self.DEBUG and leafnum > 100:
							print "fast debug"
							break
						#No channels - get the first branch leaf
						string = "(100)"
						drawstring = "%s[%i] >> %s%s" % (br_name, leafnum, br_name, string)
						tree.Draw(drawstring)
						axis = R.gPad.GetPrimitive(br_name)
						self.tree_dct[tree_name][br_name][leafnum] = bound_func(axis, Ndev)
				elif N_sl >= 100 and N_sl != 8256:
					#Many channels - leaf iterators are multivariate (i,j) instead of range(N_sl)
					matrix_dims = (N_sl/100, 100)
					#! ! ! ! ! ! ! ! method for discovering weird channel dimensions? ! ! ! !
					for i in xrange(matrix_dims[0]):
						if self.DEBUG and i > 5:
							print "fast debug"
							break
						string = "(100)"
						for j in xrange(matrix_dims[1]):
							#assigning leafnum arbitrarily???? This is a total guess.
							leafnum = i*matrix_dims[1] + (j-1)
							drawstring = "%s[%i][%i] >> %s%s" % (br_name, i, j, br_name, string)
							tree.Draw(drawstring)
							axis = R.gPad.GetPrimitive(br_name)
							#guess the value of leafnum to be the product of the iterators i,j
							self.tree_dct[tree_name][br_name][leafnum] = bound_func(axis, Ndev)
		self.tree_dct_pop = True
		return self.tree_dct

	@staticmethod
	def max_range(hist_obj, null):
		return (hist_obj.GetXaxis().GetXmin(), hist_obj.GetXaxis().GetXmax())

	@staticmethod
	def stat_range(hist_obj, N_sig):
		mean = hist_obj.GetMean()
		dev = hist_obj.GetStdDev()
		return (mean - N_sig*dev, mean + N_sig*dev)
...
#--------------------------------------------------------------------------------------------
class TwoWay(dict):
	"""A two-way dictionary for one-to-one mapping of codes and names"""
	def __init__(self, dct):
		#dict.__init__(self, )
		self.id2name = dct
		self.name2id = self.__rev()
	def __rev(self):
		rev_dct = {}
		for k, v in self.id2name.iteritems():
			rev_dct[v] = k
		return rev_dct

#--------------------------------------------------------------------------------------------
def trim_what(obj, prec=2):
	prec_str = "%5."+str(prec)+"f"
#given an unknown type (tupl, list, number), return a STRING FORMAT with numbers trimmed to prec
	if type(obj) == tuple:
		out = tuple([trim_what(thing, prec) for thing in obj])
		return out
	elif type(obj) == float:
		out = prec_str % obj
		return out
	elif type(obj) == complex:
		re = str(prec_str % obj.real) if obj.real else "0"
		im = str(prec_str % obj.imag)+"j" if obj.imag else "0"
		out = " + ".join([re, im])
		return out
	elif type(obj) == list:
		out = [trim_what(thing, prec) for thing in obj]
		return out
	else:
		#print "ping! dR.dctTools.trim_what(",obj,") does not recognize input"
		return obj

#--------------------------------------------------------------------------------------------
class PDGTools:
	"""For use with Particle Data Group Monte Carlo numbering schemes"""
	global TwoWay
	quarks = TwoWay({1: "d", 2: "u", 3: "s", 4: "c", 5: "b", 6: "t", 7: "b'", 8: "t'"})
	leptons = TwoWay({11: "_electron", 12: "electron _neutrino", 13: "_muon", 14: "muon _neutrino", 15: "_tau", 16: "tau _neutrino"})
	bosons = TwoWay({22: "gamma", 23: "Z0", 24: "W_"})
	mesons = TwoWay({111: "pi0", 211: "pi_", 311: "K0", 321: "K_", 130: "K0_L"})
	light_hadrons = TwoWay({2112: "neutron", 2212: "_proton", 3122: "Lambda", 3112: "Sigma-", 3222: "Sigma+"})

	all_parts = [quarks, leptons, bosons, light_hadrons]

	"""Masses taken from Particle Data Group Tables"""
	rest_mass = OrderedDict({2112: 939.56, 2212: 938.27, 13: 105.66})
	...
	@classmethod
	def decode_ID(self, ID, quiet=False):
	#Takes a PDG ID number and returns the particle name
		sign = (ID > 0)
		ID = int(abs(ID))
		digits = len(str(ID))
		out = None
		...
		if digits == 1:
			return self.quarks.id2name.get(ID)
		elif digits == 2:
			#Leptons
			if 11 <= ID <= 18:
				antistring = "" if (sign==True) else "anti"
				lept = self.leptons.id2name.get(ID)
				out = lept.replace("_", antistring)
			#Bosons
			elif 21 <= ID <= 37:
				antistring = "+" if sign else "-"
				bos = self.bosons.id2name.get(ID)
				out = bos.replace("_", antistring)
		#mesons
		elif digits == 3:
			antistring = "+" if (sign==True) else "-"
			mes = self.mesons.id2name.get(ID)
			try:
				out = mes.replace("_", antistring)
			except:
				print "PDG ID %i not supported" % ID

		#Light Hadrons
		elif digits == 4:
			antistring = "" if (sign==True) else "anti"
			had = self.light_hadrons.id2name.get(ID)
			try:
				out = had.replace("_", antistring)
			except:
				print "PDG ID %i not supported" % ID
		...
		#No match found
			...
			if not quiet:
				print "ID %s not currently supported by PDGTools()" % ID
			return "Unknown"

	@classmethod
	def fetch_mass(self, ID_name):
	#returns the particle mass for a given ID OR string
		if type(ID_name) is str:
			#Search for the particle in the dictionaries; abusive but it works.
			for dct in self.all_parts:
				#We have to try both 'name' and '_name'
				try1 = dct.name2id.get(ID_name)
				ID = try1 if (try1) else dct.name2id.get("_"+ID_name)
				if ID:
					break
		elif type(ID_name) is int:
			ID = ID_name

		mass = self.rest_mass.get(abs(ID))
		if mass:
			return mass
		return 0

	@classmethod
	def sister_lepton(self, ID):
	#Return the lepton counterpart for the given ID
		positive = bool(ID>0)
		#Odd numbers are big leptons
		if (abs(ID)%2):
			ID_out = abs(ID)+1
		else:
			ID_out = abs(ID)-1

		ID_out = ID_out if positive else ID_out*-1
		return ID_out

	def __access_dct(self, dct):
	#Return the identity of an incoming particle using mc PDG numbering
		return
